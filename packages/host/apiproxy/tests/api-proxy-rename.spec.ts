...
describe('sessions.refreshTitle', () => {
  it('re-runs the registered provider and returns the accepted title', async () => {
    const ctx = await composed()
    const generate = vi.fn(async () => ({ title: 'Regenerated by provider', messageSeqs: [1] }))
    ctx.sessionTitle.register({ id: 'regenerate-provider' as never, automatic: 'all-prompts', generate })
    const session = liveAgent(ctx, 'refresh-provider', 1)
    const api = createApiProxy(ctx, { defaultModelSelection: () => ({ provider: 'p', model: 'm' }), cwd: '/tmp' })

    const response = await api.sessions.refreshTitle(request({ sessionId: session.id }))
    expect(response.result).toEqual({
      ok: true,
      value: { title: 'Regenerated by provider', seq: session.events.at(-1)?.seq },
    })
    expect(generate).toHaveBeenCalledOnce()
  })

  it('resolves absent when the session has no eligible text', async () => {
    const ctx = await composed()
    const session = ctx.sessions.create(sid('refresh-empty'), { meta: { cwd: '/proj' } })
    ctx.agents.register({ id: session.id, session, status: 'idle', ctx } as Agent)
    const api = createApiProxy(ctx, { defaultModelSelection: () => ({ provider: 'p', model: 'm' }), cwd: '/tmp' })

    const response = await api.sessions.refreshTitle(request({ sessionId: session.id }))
    expect(response.result).toEqual({ ok: true, value: { absent: true } })
  })

  it('maps an unknown session to the shared session-not-found error', async () => {
    const ctx = await composed()
    ctx.provide('sessionPersistence', { list: () => Promise.resolve([]) } as never)
    const api = createApiProxy(ctx, { defaultModelSelection: () => ({ provider: 'p', model: 'm' }), cwd: '/tmp' })
    const response = await api.sessions.refreshTitle(request({ sessionId: sid('refresh-void') }))
