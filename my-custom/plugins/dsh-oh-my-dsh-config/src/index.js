...
/**
 * 侧栏品牌块自身元素的样式。只负责本组件（客户端半边 lib/client.js）渲染的
 * [data-ohmy-brand] 与 .omd-brand-*（简单属性/类选择器，全浏览器支持，无需 :has()）。
 * 外壳 .logoRow/.brandIdentity/.brandName 的固定盒子几何由客户端组件挂载后直接设
 * inline style 放宽（见 lib/client.js 的 relaxAncestors）——哈希类名 + 老浏览器
 * 不支持 :has()，所以不在这里做祖先几何。
 */
const BRAND_STYLE = [
  '[data-ohmy-brand]{display:flex;flex-direction:column;align-items:flex-start;gap:2px;min-width:0}',
  '[data-ohmy-brand] .omd-brand-line{font-size:17px;font-weight:600;letter-spacing:.04em;line-height:18px;white-space:nowrap}',
  '[data-ohmy-brand] .omd-brand-meta{font-family:var(--ds-font-family-code);font-size:9px;font-weight:500;line-height:12px;color:var(--dsw-alias-label-secondary);white-space:nowrap}',
].join('\n')
...
function apply(ctx, config) {
  ...
  ctx.on('webserver/index-inject', (table) => {
    ...
    table.push(
      ...
      // 侧栏多行品牌块的几何放宽（客户端半边 lib/client.js 读同一全局并注册
      // sidebar.brand.name 槽；此样式只放大外壳盒子，不注入任何品牌文案）。
      { kind: 'style', text: BRAND_STYLE },
    )
  })
}
