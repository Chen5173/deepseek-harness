...
- **Commit messages in this personal fork are Chinese** (subject AND body — keep the detailed body, translated), opening with the version line as the FIRST row: `【个人定制版本号】: cv.<major>.<minor>.<patch>`, then a blank line, then the Chinese subject (the version = `git rev-list --count HEAD ^<base>` mapped to `cv.1.0.<count-1>` — `cv.1.0.0` after the custom series is squashed to one commit, then +1 patch per commit; aligned with the Web/CLI `cv.1.0.x` version; see my-custom/README.md 定制 5). Example: `【个人定制版本号】: cv.1.0.1` + blank line + `Feat: 会话行三点菜单「打开工作区目录」` + detailed Chinese body.
